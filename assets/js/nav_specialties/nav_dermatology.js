// assets/js/nav_dermatology.js

const navDataUrl = 'assets/data/navigation-menu-dermatology.json'; // Path to your specific dermatology navigation data
...
async function buildAndDisplayNavigation() {
    ...
    if (!navPanel) {
        console.warn('Dermatology Navigation: Panel element (.site-navigation-panel) not found.');
        return;
    }
    ...
    try {
        ...
        if (!Array.isArray(navData.sections) || navData.sections.length === 0) {
            console.warn('Dermatology Navigation: Data is empty or not in the expected format.');
        ...
        navData.sections.forEach(section => {
            ...
            if (section.title) {
                const sectionTitleObj = section.title; // e.g., {"en": "Dermatology", "es": "Dermatologia"}
                ...
                // Special handling for "dermatology" title with Willam image
                if (sectionTitleObj.en === "Dermatology") { // Compare against English title from its own JSON
                    const charcotImageUrl = "https://upload.wikimedia.org/wikipedia/commons/7/77/Robert_willan.jpg";
                    const charcotWikipediaUrl = "https://en.wikipedia.org/wiki/Robert_Willan";
        ...
        console.error('Dermatology Navigation: Failed to build navigation:', error);
...
// window.dermatology, window.initCardiologyNavigation, etc.,
